...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        ...
        const isRecipeSaved = favorites.some(
          (fav: Favorite) => fav.recipeId === parseInt(Array.isArray(recipeId) ? recipeId[0] : recipeId)
        );
    ...
    const loadRecipeDetail = async () => {
      setLoading(true);
      try {
        const mealData = await MealAPI.getMealById(Array.isArray(recipeId) ? recipeId[0] : recipeId);
        if (mealData) {
  ...
  const getYouTubeEmbedUrl = (url: string) => {
     ...
     // handle both youtube.com/watch?v= and youtu.be/ links
     const regex = /(?:youtube\.com\/.*v=|youtu\.be\/)([^&]+)/;
     const match = url.match(regex);
     const videoId = match ? match[1] : null;

     return videoId
       ? `https://www.youtube-nocookie.com/embed/${videoId}?rel=0&autoplay=0`
       : null;
  };
    ...
    try {
      if (isSaved) {
        ...
        const response = await fetch(`${API_URL}/favorites`, {
          ...
          body: JSON.stringify({
            userId,
            recipeId: parseInt(Array.isArray(recipeId) ? recipeId[0] : recipeId),
            title: recipe?.title,
            image: recipe?.image,
          ...
          <View style={recipeDetailStyles.floatingButtons}>
            ...
            <TouchableOpacity
              style={[
                ...
                { backgroundColor: isSaving ? '#999999' : COLORS.primary },
              ]}
